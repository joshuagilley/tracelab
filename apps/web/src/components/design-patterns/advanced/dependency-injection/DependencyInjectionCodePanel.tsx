...
import baseStyles from '@/components/code/CodePanel.module.css'
...
export default function DependencyInjectionCodePanel({ files }: Props) {
  ...
  const [kind, setKind] = React.useState<CodeKind>('good')
    ...
    <div className={`panel ${baseStyles.panel}`}>
      ...
      {kind === 'bad' && (
        <p className={styles.warnBanner}>
          Anti-pattern: one config struct, <code>if/else</code> in the service, and every new transport pollutes the
          same type.
        </p>
      )}
