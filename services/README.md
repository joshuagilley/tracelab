...
- **Serves** the web app under one route family: **`GET /api/sections/{system-design|design-patterns|data-science|database-design}/concepts`** and **`.../concepts/{slug}`**. Catalog JSON lives in `internal/lessons/data/*.json`; lesson source is embedded from `internal/lessons/embed/{section}/` as **`present.*`** and **`bad.*`** only. Runnable **sandboxes** stay under repo-root **`labs/`**, not in the API image.
